...
const InputPlaces: FC<IInputPLaces> = (props) => {
	...
	const { placeholder, className, cbSuccess } = props;
	const {
		value,
		init,
		...
		data.map((suggestion) => {
			...
			return (
				<li
					...
					className="bg-blue-400 h-10 rounded-md px-3 py-2 mb-2 text-white text-ellipsis overflow-hidden last:mb-0">
	...
	return (
		<div className={cn("w-full relative", className)} ref={ref}>
			...
			<input
				...
				className={cn(
					"w-full py-2 px-4 rounded-md outline-0 border-0 font-medium placeholder:text-gray-600 transition-all duration-500",
			...
			/>
			{userPosition && (
				<button className="bg-blue-400 leading-3 py-1 text-xs font-medium w-16 text-white rounded-md absolute top-1 right-3">
					Current position
				</button>
			)}
		</div>
